...
from perception.regions import resolve_region  # noqa: E402
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Capture a named screen region without sending controller input.")
    parser.add_argument("--profile", "-p", help="Profile name or path.")
    parser.add_argument("--config", "-c", help="Explicit config YAML path.")
    parser.add_argument("--region", required=True, help="Named region to capture.")
...
def main(argv: list[str] | None = None) -> None:
    ...
    result = backend.capture_png(
        ...
        metadata={
            ...
            "region": {
                "left": named_region.left,
                "top": named_region.top,
                "width": named_region.width,
                "height": named_region.height,
                "description": named_region.description,
                "tags": list(named_region.tags),
            },
            "backend": screenshot_config.backend,
        },
    )

    print(f"Captured region: {named_region.name}")
